...
fn save_as_and_list_directory_round_trip() {
    ...
    let saved = host_handlers::save_document_as(
        &state,
    ...
    assert_eq!(
        names,
        vec!["renamed.js".to_string(), "source.js".to_string()]
    );
}
...
fn tauri_dist_contains_shell_and_worker_assets() {
    ...
    assert!(
        asset_names
            .iter()
            .any(|name| name.starts_with("ts.worker-") && name.ends_with(".js")),
...
fn end_to_end_edit_undo_redo_save_workflow() {
    ...
    // 2. Apply edits
    let edit_response = host_handlers::apply_edits(
        &state,
    ...
    // 3. Undo
    let undo_response = host_handlers::undo(
        &state,
        host::UndoRequest {
            resource: Some(file_uri(&path)),
        },
    ...
    // 4. Redo
    let redo_response = host_handlers::redo(
        &state,
        host::RedoRequest {
            resource: Some(file_uri(&path)),
        },
    ...
    let snap = host_handlers::get_buffer_snapshot(
        &state,
    ...
    let _ = host_handlers::save_document(
        &state,
...
fn multi_view_buffer_sync_via_registry() {
    ...
    let v1 = host_handlers::open_document(
        &state,
        host::OpenDocumentRequest {
            resource: Some(file_uri(&path)),
            create_if_missing: false,
    ...
    let v2 = host_handlers::open_document(
        &state,
        host::OpenDocumentRequest {
            resource: Some(file_uri(&path)),
            create_if_missing: false,
    ...
    let v2_snapshot = host_handlers::get_buffer_snapshot(
        &state,
        host::GetBufferSnapshotRequest {
            resource: Some(file_uri(&path)),
            at_version_id: 0,
...
fn security_sandbox_blocks_oversized_file() {
    ...
    assert!(
        ...
        "Expected sandbox limit error, got: {}",
        err
    );
