...
use crate::syntax::{collect_completions, collect_syntax_diagnostics, extract_document_symbols, extract_folding_ranges, hover_at_position, pack_semantic_tokens, prefix_at_position, SyntaxParser, tokenize_tree, tokenize_tree_range};
...
pub fn tokenize_document(
    ...
    let version_id = registry
        .get_buffer_version(&resource.path)
        .unwrap_or(1);
...
/// Collects syntax diagnostics from a buffer using Tree-sitter.
pub fn diagnostics_document(
    ...
    let diagnostics = collect_syntax_diagnostics(&parsed.tree, source_label);

    let proto_diagnostics = diagnostics
        .into_iter()
        .map(|d| language::Diagnostic {
...
/// Produces completion items for a buffer using Tree-sitter document symbols.
...
mod tests {
    ...
    fn tokenize_buffer_content() {
        ...
        assert!(response.tokens.iter().any(|t| t.text == "fn" && t.token_type == "keyword"));
        assert!(response.tokens.iter().any(|t| t.text == "main" && t.token_type == "identifier"));
        assert!(response.tokens.iter().any(|t| t.text == "let" && t.token_type == "keyword"));
        assert!(response.tokens.iter().any(|t| t.text == "1" && t.token_type == "number"));
    }
    ...
    fn fold_buffer_content() {
        ...
        assert!(response.ranges.iter().any(|r| r.start_line == 1 && r.end_line == 3));
        assert!(response.ranges.iter().any(|r| r.start_line == 5 && r.end_line == 8));
    }
    ...
    fn diagnostics_detects_syntax_error() {
        ...
        assert!(!response.diagnostics.is_empty());
        assert!(response.diagnostics.iter().any(|d| d.severity == language::DiagnosticSeverity::Error as i32));
    }
    ...
    fn document_symbols_buffer_content() {
        ...
        assert!(response.symbols.iter().any(|s| s.name == "main" && s.kind == "function"));
        assert!(response.symbols.iter().any(|s| s.name == "Point" && s.kind == "struct"));
